...
def load_icvl(
    *,
    ...
    if mode == "test":
        dataset = ICVLTest(resolution=1024, arg=False,)
    elif mode == "train":
        dataset = ConcatDataset([ICVLTrain(resolution=256, arg=True), ICVLValidation(resolution=256, arg=True)])
    else:
...
class ICVLBase(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        example = {
            "hsi": np.ascontiguousarray(hyper),
