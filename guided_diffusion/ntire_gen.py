...
def load_ntire2022(
    *,
    ...
    if mode == "train":
        dataset = NTIRETrain(data_root, crop_size, True, shard_id=MPI.COMM_WORLD.Get_rank(), num_shards=MPI.COMM_WORLD.Get_size())
    elif mode == "test":
        dataset = NTIREValidate(data_root, True)
    else:
...
class NTIRETrain(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=True, stride=8, shard_id=0, num_shards=1):
        ...
        hyper_list.sort()
        bgr_list.sort()
        hyper_list = hyper_list[shard_id::num_shards]
        bgr_list = bgr_list[shard_id::num_shards]
...
class NTIREValidate(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bgr = self.bgrs[idx]
        return {"rgb": np.ascontiguousarray(bgr), "hsi": np.ascontiguousarray(hyper)}
