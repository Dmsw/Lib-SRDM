...
cfg = {
    ...
    'use_ddim': False,
    'n_samples': 1,
    'save_dir': "/home/root/project/hsi-denoising/GDM/results/Ablation/wosam5/",
    'spatial_kwargs' : {
    ...
    'library_kwargs': {
        'lib_path': "/home/root/project/hsi-denoising/GDM/library/cave_all/lib.pkl",
        'samples_per_tag': 100,
    },
    'l1': 3,
...
def main():
    ...
    while count * cfg['batch_size'] < cfg['num_samples']:
        ...
        if cfg['spectral_kwargs'] is not None:
            ...
            rgb = (model_kwargs['ref_img'] * 0.5 + 0.5).numpy().reshape([3, -1]).T
            lib = pickle.load(open(cfg['library_kwargs']['lib_path'], "rb"))
            lib = np.concatenate([v for v in lib.values()], axis=0)
            lib = {"default":sample_by_rgb(rgb, lib, srf.P.T, cfg['library_kwargs']['samples_per_tag'], cfg['tau'])}
            mask = {"default":np.ones([512, 512], dtype=np.bool)}
            factor = np.ones([512, 512], dtype=np.float32)
            ...
            if cfg['k_svd'] > 0:
                lr_grad = LowRankGradients(lib, mask, factor, diffusion.alphas_cumprod, cfg['k_svd'])
            spectral_model = PriorModels(lib, mask, factor, bar_alpha=spectral_diffusion.alphas_cumprod)
